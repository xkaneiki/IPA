...
/*insert operation*/
struct myrb_node *myrb_insert(myrb_tree *T, KElemType key)
{
    ...
    return myrb_node_insert(T, node);
}

struct myrb_node *myrb_node_insert(myrb_tree *T, struct myrb_node *node)
{
    // case 1: node is root
    ...
    {
        // myrb_print(*T);
        return node; //T has been a rb tree
    }
